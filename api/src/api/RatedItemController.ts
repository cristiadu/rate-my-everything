...
export default class RatedItemController {
  ...
  public async getById(req: Request, res: Response) {
    try {
      ...
      if (data) {
        ...
        res.status(404).json(NewApiError('NOT_FOUND', 404, 'Rated item not found'))
      }
    } catch (error) {
      console.error(error)
      res.status(500).json(NewApiError('INTERNAL_ERROR', 500, 'An internal server error occurred'))
  ...
  public async create(req: Request, res: Response) {
    try {
      ...
      if (!item_id || !user_id || rating === undefined) {
        return res.status(400).json(NewApiError('VALIDATION_ERROR', 400, 'Missing required fields: item_id, user_id, rating'))
      }
